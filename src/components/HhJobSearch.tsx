...
const HhJobSearch: React.FC = () => {
  ...
  const changePage = (pageChange: number) => {
    const newPage = Math.max(
      0,
      Math.min((data?.pages || 1) - 1, (searchParams.page || 0) + pageChange)
    );
  ...
  const formatSalary = (
    ...
    if (salary.to !== null)
      result += `${
        salary.from !== null ? "до" : "До"
      } ${salary.to.toLocaleString("ru-RU")} `;
  ...
  const renderMainAreas = () => {
    ...
    const russiaTyped: Area = russia as Area;

    return russiaTyped.areas.map((area: Area) => (
      ...
      {isLoading ? (
        ...
        <>
          <div className="mb-4">
            <p>
              Найдено вакансий:{" "}
              <strong>{data.found.toLocaleString("ru-RU")}</strong>
            </p>
            <p>
              Страница {(searchParams.page || 0) + 1} из {data.pages}
            </p>
          </div>
                ...
                >
                  ...
                  <div className="mt-3">
                    ...
                    <div className="text-sm text-gray-600 mt-1">
                      {vacancy.area.name} • {vacancy.experience.name} •{" "}
                    ...
                    <div className="mt-3 text-gray-700">
                      {vacancy.snippet.requirement && (
                        <div className="mb-2">
                          ...
                          {vacancy.snippet.requirement.replace(
                            /<highlighttext>|<\/highlighttext>/g,
                            ""
                          )}
                        </div>
                      )}
                      {vacancy.snippet.responsibility && (
                        <div>
                          ...
                          {vacancy.snippet.responsibility.replace(
                            /<highlighttext>|<\/highlighttext>/g,
                            ""
                          )}
                        </div>
